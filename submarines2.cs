...
namespace _1904tar3
{
    class Program
    {
        ...
        static void Targi3()
        {
            ...
            int[,] guesses_matrix = new int[rows, columns];
            ...
            do
            {
                ...
                for (int i = 0; i < matrix.GetLength(0); i++)
                {
                    for (int j = 0; j < matrix.GetLength(1); j++)
                    {
                        if (guesses_matrix[i, j] == 0)
                        {
                            Console.Write("X" + " ");
                        }
                        else if(guesses_matrix[i, j] == 1)
                        {
                            Console.Write("0" + " ");
                        }
                        else if (guesses_matrix[i, j] == 2)
                        {
                            Console.Write("S" + " ");
                        }
                ...
                if (matrix[user_guess_row_not_zero_based - 1, user_guess_col_not_zero_based - 1] == 1)
                {
                    ...
                    submarines--;
                    guesses_matrix[user_guess_row_not_zero_based - 1, user_guess_col_not_zero_based - 1] = 2;
                }
                else
                {
                    // miss
                    Console.WriteLine(".... You missed ....");
                    guesses_matrix[user_guess_row_not_zero_based - 1, user_guess_col_not_zero_based - 1] = 1;
                }
            }
            while (submarines > 0);
